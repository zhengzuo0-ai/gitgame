...
### 逝者的回响（graveyard inheritance）

新角色可以在世界中遭遇已死角色留下的痕迹——但**绝非继承力量**：

- NPC 可能提到已死角色的名字（"上一个来的人没能回去"）。
- 新角色可能在地点里看到上一位角色丢弃的物品残骸（纯叙事，不可拾取）。
- 如果已死角色有 `mist-touched` 状态，Lyra 可能对新角色说一句关于前任的话。
- `/roll-character` 时如果 Graveyard 非空，GM **应该**在角色起源叙事中暗示
  一句与这片土地上的死者有关的细节（"他听说过一个再也没回来的名字"）。
...
- [ ] 回合末尾提示玩家可用选项（见下方"回合末尾提示"）

---

## 回合末尾提示（强制）

每回合叙事结束后，**必须**附带一行可用选项提示（不算在叙事句数内）。
格式为一行列表，用 `·` 分隔：

- 在 skirmish 中（不可中断）：`行动 · 对话 · 观察`
- 在 expedition 中：`行动 · 对话 · 观察 · /rest 撤退`
- 在 saga 中：`行动 · 对话 · 观察 · /inventory 查看装备`

不要解释每个选项的含义——只列出。玩家打字说任何话都算合法输入，
这个列表只是提醒他们"你可以开口了"。
...
存活角色在冒险结束时有最低奖励保证——骰运不应让成功毫无收获。

| 档位 | 存活保底 |
|---|---|
| skirmish | 可以无战利品（骰点 1–10 = 空手） |
| expedition | **至少 1 件 common** — 若 `loot_d20` ≤ 5，改判为 6（1 common/uncommon） |
| saga | **至少 1 件 uncommon + 至少 80 XP** — 若 `loot_d20` ≤ 3，改判为 4 |

改判时在日志里注明 `[保底生效: 原骰 N → 调整为 M]`。

---

## Saga 致命一击上限

Boss turn 的"伤害翻倍"规则补充：

- 翻倍后的伤害**不得超过 hp_max 的 75%**。即使翻倍后算出更高，也截止到 75%。
- 这意味着满血角色不会被单次 boss hit 秒杀（除非他们进 boss turn 时已经受伤）。
- 截止时在日志里注明 `[致命一击上限: 原伤 N → 截止为 M]`。

例：hp_max = 14，boss 翻倍后伤害 = 12 → 截止为 `floor(14 * 0.75) = 10`。

此规则**只影响 boss turn 翻倍伤害**，普通伤害不受限。
